...
uint256 hashGenesisBlock("0x326f05dbe2ecad2846af02cf6cb506a4b2033f1aa3801f5f4061acefc31ce109");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Errorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Errorcoin Signed Message:\n";
...
{
    // Errorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 1001 * COIN;
    if(nHeight == 2)  
    {
        nSubsidy = 24300000 * COIN;
    }
    ...
    nSubsidy >>= (nHeight / 840000); // Errorcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 660; // Errorcoin: 11 minutes
static const int64 nTargetSpacing = 1 * 61; // Errorcoin: 61 Seconds.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Errorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x326f05dbe2ecad2846af02cf6cb506a4b2033f1aa3801f5f4061acefc31ce109");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2015-07-01 Images show Chinese airstrip on man-made Spratly island nearly finished";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x9329e2335b83f1397802482527b312bb9a3621bb67a50794387f6e71ff1adb55"));
...
unsigned char pchMessageStart[4] = { 0xd4, 0xe3, 0xe2, 0xce }; // Errorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ErrorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ErrorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ErrorcoinMiner(CWallet *pwallet)
{
    printf("ErrorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("errorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ErrorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ErrorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ErrorcoinMiner, pwallet));
}
